...
// GetAddressBalance ...
func (c *Client) GetAddressBalance(address []string) (*Balance, error) {
	cmd := NewGetAddressBalanceCmd(&AddressesRequest{Addresses: address})
...
func (c *Client) GetAddressUTXOs(address []string) ([]UTXO, error) {
	cmd := NewGetAddressUTXOsCmd(&AddressesRequest{Addresses: address})
	resBytes, err := c.sendCmd(cmd)
	if err != nil {
		return nil, err
	}
